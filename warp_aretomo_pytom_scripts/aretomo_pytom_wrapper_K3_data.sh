...
do 
	...
	newstack -size 4092,4092  "$i/$i.st" "$i/${i%.mrc}_crop.st"
	aretomo -InMrc "$i/${i%.mrc}_crop.st" -OutMrc "$i/${i%.mrc}_crop.mrc" -AngFile "$i/$i.rawtlt" -VolZ 1200 -AlignZ 800 -Wbp 1 -FlipVol 1 -OutBin 8 -Gpu 0 -TiltAxis 84.5 -DarkTol 0.01
	...
	# apply alignment from square stack to the full K3 images
	echo "applying alignment cropped $i"
	aretomo -InMrc "$i/$i.st" -OutMrc "$i/$i" -AlnFile "$i/${i%.mrc}.aln" -VolZ 1200 -Wbp 1 -FlipVol 1 -OutBin 8 -Gpu 0 -OutImod 2
	mv "${i%.mrc}_projXY.mrc" "$i"
	mv "${i%.mrc}_projXZ.mrc" "$i"
